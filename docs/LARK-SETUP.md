...
消息卡片的交互操作（如下拉框选择、按钮点击）需要配置回调地址，否则用户操作卡片时会收到 **错误 200340**。

> ⚠️ **重要**: 必须使用**新版机器人**才能配置回调地址。如果相关配置项显示为灰色（不可编辑），请先升级机器人版本。

**配置步骤：**

1. 在应用管理页面，进入 **事件与回调** 配置
2. 找到 **回调配置** 部分
3. 订阅方式选择：**将回调发送至 开发者服务器**
4. 填入请求地址（与事件配置的 Webhook URL 相同）：
   ```
   https://xxxxxxxxxx.execute-api.us-east-1.amazonaws.com/prod/messages
   ```

> 💡 **说明**:
> - **事件配置**（请求地址）：处理消息接收等事件（如 `im.message.receive_v1`）
> - **回调配置**（请求地址）：处理卡片交互回调（如 `card.action.trigger`）
> - 两者使用相同的 Webhook URL，但必须分别配置
>
> ⚠️ **如果回调配置显示为灰色**：说明当前使用的是旧版机器人框架。请在 Lark 开放平台将机器人升级为新版本，升级后回调配置将变为可编辑状态。
...
如遇问题，请参考 [TROUBLESHOOTING.md](TROUBLESHOOTING.md) 获取完整的故障排查指南。

常见问题包括：
- 收不到消息
- 无法创建群
- 用户名显示为"团队成员"
- 点击卡片下拉框报错 200340（需配置回调地址，详见 [5.3](#53-配置消息卡片回调地址)）
